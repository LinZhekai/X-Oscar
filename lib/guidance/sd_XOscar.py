...
class StableDiffusion(nn.Module):
    ...
    def train_step(self, text_embeddings, pred_rgb, guidance_scale=100, rgb_as_latents=False,geo_std=0,app_std=0):
        if rgb_as_latents:
        ...
        with torch.no_grad():
            # add noise
            noise = 0.8*torch.randn_like(latents) + 0.1*geo_std*torch.randn_like(latents) + 0.1*app_std*torch.randn_like(latents)
